...
from srcs.model.poisson_deblurring_modules import ResUNet
from srcs.model.poisson_deblurring_utils import pad, conv_fft, conv_fft_batch, psf_to_otf
...
class Z_Update_ResUNet(nn.Module):
	def __init__(self):
		...
		self.net = ResUNet(in_nc=3, out_nc=3)  # 1, 3 for grey or color images
...
class InitNet(nn.Module):
	...
	def forward(self, kernel, M):
		...
		x = torch.cat(
			(x.view(N, 1, 16*8*8),  M.float().view(N, 1, 1)), axis=2).float()
		h = self.mlp(x)+1e-6
...
class P4IP_Net(nn.Module):
	def __init__(self, n_iters=8, device='cuda'):
		...
		self.Z = Z_Update_ResUNet()  # BW Denoiser / color denoiser
