...
p4ip = P4IP_Net()
p4ip.load_state_dict(torch.load(MODEL_FILE))
p4ip.to(device)
p4ip.eval()
...
kernel = 255*k.astype(np.float32)/k.max()
cv2.imwrite('results/p4ip_'+"%02d" % IDX+'.png', x_p4ip,
            [cv2.IMWRITE_PNG_COMPRESSION, 0])
cv2.imwrite('results/y_demosaic_'+"%02d" % IDX+'.png',
            y, [cv2.IMWRITE_PNG_COMPRESSION, 0])
cv2.imwrite('results/kernel_'+"%02d" % IDX+'.png',
            kernel, [cv2.IMWRITE_PNG_COMPRESSION, 0])
...
# plt.subplot(1, 3, 2)
# plt.imshow(np.flip(y_cv, axis=2))
# plt.axis('off')
# plt.title('Noisy Blurred Image')
# plt.subplot(1, 3, 3)
# plt.imshow(np.flip(im_estimated, axis=2))
# plt.axis('off')
